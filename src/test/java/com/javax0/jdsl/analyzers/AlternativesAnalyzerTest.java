...
public class AlternativesAnalyzerTest {
    ...
    private void given_AlternativesAnalyzerNFailing1SuccessFullMFailing_then_OnlyUpToSuccessFulIsCalled(int n, int m) {
        ...
        WHEN: {
            for (int i = 0; i < n; i++) {
                ...
                Mockito.when(analyzers[i].analyze(Mockito.any(SourceCode.class))).thenReturn(SimpleAnalysisResult.failed());
                analyzer.add(analyzers[i]);
            }
            successfulAnalyzer = Mockito.mock(Analyzer.class);
            Mockito.when(successfulAnalyzer.analyze(Mockito.any(SourceCode.class))).thenReturn(SimpleAnalysisResult.success(null, null));
        ...
        THEN: {
            for (int i = 0; i < n; i++) {
                Mockito.verify(analyzers[i]).analyze(Mockito.any(SourceCode.class));
            }
            Mockito.verify(successfulAnalyzer).analyze(Mockito.any(SourceCode.class));
            for (int i = n; i < n + m; i++) {
                Mockito.verify(analyzers[i], Mockito.never()).analyze(Mockito.any(SourceCode.class));
            }
    ...
    public void given_AnAlternativesAnalyzer_when_FirstAlternativeMatches_then_ReturnsSuccessful() {
        for (int n = 0; n < 2; n++) {
            for (int m = 0; m < 2; m++) {
                given_AlternativesAnalyzerNFailing1SuccessFullMFailing_then_OnlyUpToSuccessFulIsCalled(n, m);
            }
        }
    }
